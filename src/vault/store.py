"""T4 PII Vault: Fernet 暗号化 + audit log (T1/T2/T3 inherit、 internal ADR § 3 PII/Op 分離 順守)。
...
は本 module literal import 禁止 (drift-check CI で boundary check)。
...
def emit_audit(action: str, item_id: str, requester: str = "system", reason: str = "") -> None:
    """全 vault access を audit log に append (T1/T2/T3 inherit、 ADR-007 layer 7)。"""
