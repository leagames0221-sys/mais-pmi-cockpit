"""T4 Object Type Pydantic schema SSoT (ADR-303 順守、 9 Object Type literal 定義)。

internal ADR § 3 PII/Op 分離 pattern 適用:
...
# ─── ID type aliases (link key UUID-style、 internal ADR inherit) ────────────
...
# T3 ID prefix (T3 → T4 mapping reference 用、 ADR-302 順守)
T3IntegrationPlanId = str  # IP-XXXXXXXXX (T3 ADR-202/203)
T3CommunicationKitId = str  # CK-XXXXXX (T3 ADR-203)
T3JPDay1PatternId = str  # JPD1-XXXXXX (T3 ADR-205)


# ─── 1. CockpitProject (CP-XXXXXX、 PMI 案件 1 件、 T3 IP-XXXXXX inherit) ─

class CockpitProject(BaseModel):
    """PMI 案件 1 件 = 1 CP、 Day-1 → Day-100 期間の cockpit root。 ADR-302 順守、 T3 IntegrationPlan 1:1 inherit reference。"""
...
class DriverInsight(BaseModel):
    ...
    statement_redacted: str  # 担当者氏名 / 取引先名 / 内部金額 redact 済 (internal ADR inherit)
...
class NextActionCandidate(BaseModel):
    """NextAction 5 候補 ranked (Claude LLM listwise CoT、 ADR-301 順守)。"""
...
class NextAction(BaseModel):
    ...
    audience_mapping: T3CommunicationKitId = Field(..., pattern=r"^CK-[0-9]{6}$")  # T3 CommunicationKit literal reference (ADR-302)
...
class VendorContract(BaseModel):
    """vendor 契約 entity (Docling parse 出力、 T2 ADR-101 inherit)、 重複統合機会 detect 対象。"""
...
class RetentionRisk(BaseModel):
    ...
    triggered_jp_patterns: list[T3JPDay1PatternId] = Field(default_factory=list)  # T3 JPDay1Pattern reference (ADR-302)
...
# ─── T4Output container (full T3 → T4 mapping result、 ADR-302 順守) ────

class T4Output(BaseModel):
    """T3 API output → T4 ingestion 全 結果の container schema (ADR-302 mapping table SSoT)。"""
