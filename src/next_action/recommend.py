"""NextAction recommender (Claude LLM listwise CoT + 5 候補 ranked + T3 CK audience_mapping reference、 ADR-304 順守)。

flow:
  1. DriverInsight → LLMProvider.generate_next_action_candidates で 5 候補 ranked
  2. T3 CommunicationKit (CK-XXXXXX) から audience_mapping を literal reference (ADR-302 mapping rule)
...
def _due_day_n(current_day_n: int, dimension: str) -> int:
    """dimension-specific buffer で due_day_n 算出 (Week 2 PoC、 ADR-305 で 中堅日本企業 fit で literal 拡張)。"""
    BUFFER_BY_DIM = {
        "cost": 14,
        "revenue": 14,
