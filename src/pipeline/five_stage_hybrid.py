"""5-stage hybrid pipeline (T1 ADR-005 inherit、 BM25 + dense + RRF + cross-encoder + LLM listwise、 Week 3 active)。

Stage 構成 (T1 ADR-005 inherit):
...
def stage3_rrf(results: list[HybridResult], k: int = 60) -> list[HybridResult]:
    """BM25 rank + dense rank を RRF score に literal fuse (T1 ADR-005 順守、 k=60 = literature default)。"""
    # BM25 rank
...
def stage5_llm_listwise_rerank(results: list[HybridResult], top_k: int = 5) -> list[HybridResult]:
    """LLM listwise CoT で最終 ranking (T1 ADR-005 順守、 試作 = cross_encoder_score 順)。
