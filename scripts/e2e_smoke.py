"""T4 e2e smoke (18 step pattern、 T1/T2/T3 inherit + ADR-303/304/305 全 layer literal verify)。
...
def main() -> int:
    print("=== T4 e2e smoke (18 step、 ADR-303/304/305 全 layer literal verify) ===\n")

    # Phase 1: data_gen (合成 T4Output baseline)
    print("[Phase 1: synthetic data gen (ADR-303 data_gen layer)]")
    ...
    # Phase 2: anomaly detect (ADR-304 Isolation Forest)
    print("\n[Phase 2: Isolation Forest anomaly detect (ADR-304)]")
    all_anomalies = []
    ...
    # Phase 3: vendor + SaaS overlap (ADR-305 中堅日本企業 pattern)
    print("\n[Phase 3: vendor + SaaS overlap detect (ADR-305)]")
    ...
    print("\n[Phase 5: vault (Fernet) + Superset embed (ADR-301)]")
    import os
